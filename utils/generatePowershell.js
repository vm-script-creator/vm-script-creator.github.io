...
function setStaticIP(iF, ip, mask, gateway) {
  return `netsh interface ip set address "${iF}" static ${ip} ${mask} ${gateway}\n`
}
function setDNS(iF, dns, index) {
  return `netsh interface ip set dns name="${iF}" static ${dns} ${index == 1 ? "primary" : "both"} validate=no\n`
}
...
export default function generatePowershell(props) {
  return new Promise((resolve, reject) => {
    ...
    // NETWORKING IDK
    if (!props.networking.useDHCP) {
      powershell1+= setStaticIP(props.networking.iF, props.networking.staticConf.ip, props.networking.staticConf.subnet, props.networking.staticConf.gateway);
    }
    if (props.networking.dns1 != "") {
      powershell1+= setDNS(props.networking.iF, props.networking.dns1, 1);
    }
    if (props.networking.dns2 != "") {
      powershell1+= setDNS(props.networking.iF, props.networking.dns2, 2);
    }
    ...
    if (props.isServer) {
      ...
      if (props.ADDS.install) {
        powershell2 += 'Install-windowsfeature -name AD-Domain-Services –IncludeManagementTools\n';
